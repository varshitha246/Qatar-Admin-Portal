...
async function handleLogout() {
    try { await fetch('http://localhost:5000/api/logout', { method: 'POST', credentials: 'include' }); } catch(e) {}
...
function openOpportunityModal() {
    ...
    // Only reset form for new opportunity (not edit)
    if (!editingOpportunityId) {
        document.getElementById('opportunityForm').reset();
        document.getElementById('opportunityModalTitle').textContent = 'Add New Opportunity';
        document.getElementById('opportunityFormSubmitBtn').textContent = 'Create Opportunity';
    }
}
...
function buildOpportunityCard(opp) {
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            ...
            <button class="view-course-btn" style="width:auto;padding:8px 12px;" onclick="viewOppDetails(this)">View Details</button>
            <button class="view-course-btn" style="width:auto;padding:8px 12px;background:#2563eb;" onclick="editOpp(this)">Edit</button>
            <button class="view-course-btn" style="width:auto;padding:8px 12px;background:#dc2626;" onclick="deleteOpp(this)">Delete</button>
        </div>
    `;
    return card;
}
...
async function viewOppDetails(btn) {
    const card = btn.closest('.opportunity-card');
    const id = card.getAttribute('data-id');
    try {
        const res = await fetch('http://localhost:5000/api/opportunities/' + id, { credentials: 'include' });
        ...
        openOpportunityDetails(opp.name, {
            ...
            skills: skills,
            applicants: opp.max_applicants || 'N/A',
            futureOpportunities: opp.future_opportunities,
            prerequisites: ''
        });
    } catch(err) { showToast('Could not load opportunity details.'); }
}

async function editOpp(btn) {
    const card = btn.closest('.opportunity-card');
    const id = card.getAttribute('data-id');
    try {
        const res = await fetch('http://localhost:5000/api/opportunities/' + id, { credentials: 'include' });
        ...
        document.getElementById('opportunityFormSubmitBtn').textContent = 'Update Opportunity';
        openOpportunityModal();
    } catch(err) { showToast('Failed to load opportunity.'); }
}

async function deleteOpp(btn) {
    const card = btn.closest('.opportunity-card');
    const id = card.getAttribute('data-id');
    const name = card.querySelector('h5').textContent;
    if (!confirm('Delete "' + name + '"? This cannot be undone.')) return;
    try {
        const res = await fetch('http://localhost:5000/api/opportunities/' + id, { method: 'DELETE', credentials: 'include' });
        if (res.ok) { card.remove(); showEmptyStateIfNeeded(); showToast('Opportunity deleted.'); }
...
function showEmptyStateIfNeeded() {
    ...
    let msg = grid.querySelector('.opp-empty-msg');
    if (cards.length === 0) {
        if (!msg) {
            msg = document.createElement('p');
            msg.className = 'opp-empty-msg';
            msg.style.cssText = 'color:#888;text-align:center;padding:40px;width:100%;';
            msg.textContent = 'No opportunities yet. Click "Add New Opportunity" to create one.';
...
async function loadOpportunities() {
    try {
        const res = await fetch('http://localhost:5000/api/opportunities', { credentials: 'include' });
        if (!res.ok) return;
...
// Load opportunities when Opportunity Management tab is clicked
document.querySelectorAll('.nav-item[data-page="opportunity"]').forEach(item => {
    item.addEventListener('click', loadOpportunities);
});

// Handle opportunity form submission (create OR edit)
        document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
            ...
            const payload = { name, duration, start_date: startDate, description, skills: skillsRaw, category, future_opportunities: futureOpportunities, max_applicants: maxApplicants || null };
            const isEdit = !!editingOpportunityId;
            const url = isEdit ? 'http://localhost:5000/api/opportunities/' + editingOpportunityId : 'http://localhost:5000/api/opportunities';
            ...
            try {
                ...
                if (isEdit) {
                    ...
                    const emptyMsg = grid.querySelector('.opp-empty-msg');
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('http://localhost:5000/api/login', {
            ...
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, password, captcha: captchaInput, remember_me: rememberMe })
        });
        const data = await res.json();
        if (!res.ok) {
        ...
        setTimeout(() => { showDashboard(email); loadOpportunities(); }, 1000);
        generateCaptcha('login');
    } catch(err) {
        showError('loginPasswordErr', 'Could not connect to server. Is the backend running?');
        shakeForm('loginForm');
    }
});
...
document.getElementById('signupForm').addEventListener('submit', async function(e) {
    ...
    try {
        const res = await fetch('http://localhost:5000/api/signup', {
            ...
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ full_name: name, email, password, confirm_password: confirmPassword, captcha: captchaInput })
        });
        const data = await res.json();
        if (!res.ok) {
            showError('signupEmailErr', data.error || 'Signup failed.');
            document.getElementById('signupEmail').classList.add('error');
            shakeForm('signupForm'); generateCaptcha('signup'); return;
        }
        ...
        showError('signupEmailErr', 'Could not connect to server. Is the backend running?');
